...
export default function ProductGalleryManager({productId,slug,name}:{productId:string;slug:string;name:string}){
 ...
 async function insertImage(imageUrl:string){
  ...
  if(!clean)return;
  if(images.length>=MAX_GALLERY_IMAGES)throw new Error("MAXIMUM GALLERY SIZE IS 5.");
  try{new URL(clean)}catch{throw new Error("ENTER A VALID IMAGE URL.");}
  const nextOrder=images.length?Math.max(...images.map(x=>x.display_order))+1:0;
  const isFirst=images.length===0;
  const {error}=await supabase.from("product_images").insert({product_id:productId,image_url:clean,alt_text:name,display_order:nextOrder,is_primary:isFirst});
 ...
 async function addUrl(){if(!supabase||!url.trim())return;setBusy(true);setMessage("");try{await insertImage(url);setUrl("");await load();setMessage("IMAGE URL ADDED. 1 = PRIMARY · 2 = HOVER.")}catch(err){setMessage(err instanceof Error?err.message:"IMAGE URL COULD NOT BE ADDED.")}finally{setBusy(false)}}
 ...
 return <section className="admin-section admin-gallery-section"><div className="admin-section-title"><div><span>PRODUCT PHOTOS / {images.length} OF {MAX_GALLERY_IMAGES}</span><small className="admin-gallery-hint">ONE IMAGE BOX · LOCAL UPLOAD OR IMAGE URL · JPG / PNG / WEBP · MAX 5MB EACH</small></div></div><div className="admin-gallery-input-box"><label className="admin-gallery-dropzone"> <strong>{busy?"UPLOADING…":"ADD PRODUCT PHOTOS"}</strong><span>Choose 1–5 local photos at once</span><small>IMAGE 1 = PRIMARY · IMAGE 2 = HOVER · IMAGES 3–5 = PDP GALLERY</small><input hidden multiple type="file" accept="image/jpeg,image/png,image/webp" onChange={add} disabled={busy||images.length>=MAX_GALLERY_IMAGES}/></label><div className="admin-gallery-url-row"><input value={url} onChange={e=>setUrl(e.target.value)} placeholder="Or paste an image URL…" aria-label="Product image URL"/><button type="button" className="admin-action" onClick={addUrl} disabled={busy||images.length>=MAX_GALLERY_IMAGES||!url.trim()}>ADD URL +</button></div></div>{message&&<div className="admin-empty">{message}</div>}<div className="admin-gallery-grid">{images.map((image,index)=><div className="admin-gallery-card" key={image.id}><div className="admin-gallery-media"><img src={image.image_url} alt={image.alt_text||name}/><span>{image.is_primary?"PRIMARY":index===1?"HOVER":"GALLERY"}</span></div><div><strong>IMAGE {index+1}</strong><small>{index===0?"Homepage default":index===1?"Homepage mouse hover":"Product gallery"}</small></div><div className="row-actions"><button type="button" className="table-action" onClick={()=>primary(image)} disabled={image.is_primary}>{image.is_primary?"PRIMARY":"MAKE PRIMARY"}</button><button type="button" className="table-action danger" onClick={()=>remove(image)}>REMOVE</button></div></div>)}</div>{images.length===0&&!message?<div className="admin-empty">NO PHOTOS YET. USE THE SINGLE BOX ABOVE FOR LOCAL PHOTOS OR IMAGE URLS.</div>:null}</section>;
}
