...
export default function ProductGalleryManager({productId,slug,name}:{productId:string;slug:string;name:string}){
 ...
 return <section className="admin-section admin-gallery-section"><div className="admin-section-title"><div><span>PRODUCT GALLERY / {images.length} OF {MAX_GALLERY_IMAGES}</span><small className="admin-gallery-hint">LOCAL UPLOAD OR IMAGE URL · JPG / PNG / WEBP · MAX 5MB EACH · 1 PRIMARY · 2 HOVER</small></div><label className="admin-action admin-upload-trigger">{busy?"UPLOADING…":images.length>=MAX_GALLERY_IMAGES?"5 PHOTOS ADDED":"ADD LOCAL PHOTOS +"}<input hidden multiple type="file" accept="image/jpeg,image/png,image/webp" onChange={add} disabled={busy||images.length>=MAX_GALLERY_IMAGES}/></label></div>{message&&<div className="admin-empty">{message}</div>}<div className="admin-gallery-url"><input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://example.com/product-image.jpg" aria-label="Product image URL"/><button type="button" className="admin-action" onClick={addUrl} disabled={busy||images.length>=MAX_GALLERY_IMAGES||!url.trim()}>ADD IMAGE URL +</button></div><div className="admin-gallery-grid">{images.map((image,index)=><div className="admin-gallery-card" key={image.id}><div className="admin-gallery-media"><img src={image.image_url} alt={image.alt_text||name}/><span>{image.is_primary?"PRIMARY":index===1?"HOVER":"GALLERY"}</span></div><div><strong>IMAGE {index+1}</strong><small>{index===0?"Homepage default":index===1?"Homepage mouse hover":"Product gallery"}</small></div><div className="row-actions"><button type="button" className="table-action" onClick={()=>primary(image)} disabled={image.is_primary}>{image.is_primary?"PRIMARY":"MAKE PRIMARY"}</button><button type="button" className="table-action danger" onClick={()=>remove(image)}>REMOVE</button></div></div>)}</div>{images.length===0&&!message?<div className="admin-empty">NO GALLERY IMAGES YET. ADD 2–5 LOCAL PHOTOS OR IMAGE URLS FOR THE HOMEPAGE HOVER AND PDP GALLERY.</div>:null}</section>;
}
