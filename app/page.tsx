...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loadCatalog = async () => {
      ...
      if (!supabase) return;
      const { data } = await supabase.from("products").select("id,slug,name,price,image_url,category_id").eq("is_active", true).order("created_at", { ascending: false });
  ...
  return <main className="site-shell" style={{ "--mx": `${pointer.x}px`, "--my": `${pointer.y}px` } as React.CSSProperties}>
    ...
    <section id="shop" className="collection"><div className="section-head"><span>01 — COLLECTION</span><span>NEW ARRIVALS / {products.length}</span></div><div className="product-grid">{products.map((product, index) => <a className="product-card" href={`/product/${product.slug}`} key={product.id}><div className={`product-image image-${String(index + 1).padStart(2, "0")} tone-${["rose", "stone", "plum", "sand"][index % 4]}`} style={product.image_url ? { backgroundImage: `url(${product.image_url})`, backgroundSize: "cover", backgroundPosition: "center" } : undefined}><span>{product.name.split(" ")[0].toUpperCase()}</span><i>VIEW</i></div><div className="product-meta"><span>{product.name}</span><span>₹ {Number(product.price).toLocaleString("en-IN")}</span></div></a>)}</div></section>
