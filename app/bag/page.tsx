...
export default function Bag(){
 ...
 return <main className="bag-page"><header className="topbar"><a className="wordmark" href="/">LE SHE<br/><span>SAREE</span></a><nav><a href="/">Shop</a><a href="/bag">Bag ({count})</a></nav><span className="top-meta">YOUR BAG</span></header><section className="bag-wrap"><div className="section-head"><span>02 — BAG</span><span>{count} ITEM{count===1?"":"S"}</span></div>{items.length===0?<div className="empty-bag"><h1>YOUR BAG<br/>IS QUIET.</h1><a href="/">Return to collection →</a></div>:<div className="bag-content"><div className="bag-list">{items.map((item,index)=>{const key=`${item.product_id}-${item.size}`;const imageUrl=displayImageUrl(item.image_url);return <article className={`bag-item ${removing===key||removing==="all"?"is-removing":""}`} key={key}><div className="bag-art" style={imageUrl?{backgroundImage:`url(${imageUrl})`,backgroundSize:"cover",backgroundPosition:"center"}:undefined}><span>{item.name.split(" ")[0].toUpperCase()}</span></div><div className="bag-copy"><p className="eyebrow">SELECTED PIECE</p><h2>{item.name}</h2><p>Size · {item.size}</p><div className="quantity"><button aria-label="Decrease quantity" onClick={()=>change(index,-1)}>−</button><strong>{item.quantity}</strong><button aria-label="Increase quantity" onClick={()=>change(index,1)}>+</button></div><strong>₹ {(item.price*item.quantity).toLocaleString("en-IN")}</strong><button className="remove-item" onClick={()=>remove(key)}>Remove piece <span>×</span></button></div></article>})}</div><aside className="bag-summary"><p className="eyebrow">ORDER SUMMARY</p><div className="bag-total"><span>Subtotal</span><strong>₹ {subtotal.toLocaleString("en-IN")}</strong></div><p>{count} piece{count===1?"":"s"} selected · Shipping calculated at checkout.</p><a className="checkout-button" href="/checkout">CHECKOUT <span>↗</span></a><button className="text-button" onClick={clear}>Clear bag</button></aside></div>}</section></main>
}
