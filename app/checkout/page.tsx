...
type CashfreeClient={checkout:(options:{paymentSessionId:string;redirectTarget:"_self"})=>Promise<unknown>};
...
async function loadCashfree(){
 ...
 if(!window.Cashfree)throw new Error("Payment gateway is unavailable");
...
export default function CheckoutPage(){
 ...
 async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setError("");if(!items.length){setError("Your bag is empty. Please add a saree first.");return;}setBusy(true);const form=new FormData(event.currentTarget);const customerName=String(form.get("name")||"");const email=String(form.get("email")||"");const phone=String(form.get("phone")||"");const address=String(form.get("address")||"");const city=String(form.get("city")||"");const pin=String(form.get("pin")||"");const supabase=getSupabaseBrowserClient();if(!supabase){setError("Store connection is not configured yet.");setBusy(false);return;}
 const {data,error:rpcError}=await supabase.rpc("place_order_with_promo",{p_customer_name:customerName,p_email:email,p_phone:phone,p_address:address,p_city:city,p_pin_code:pin,p_items:items.map(item=>({product_id:item.product_id,quantity:item.quantity,size:item.size})),p_promo_code:applied?.code||null});
