...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    ...
    params.then(async ({ slug }) => {
      ...
      if (active) {
        setBag(Number(localStorage.getItem("leshe-bag-count") || 0));
        setLoading(false);
      }
    });
    return () => { active = false; };
  ...
  const selectRelative = (delta: number) => {
    ...
    const next = (selectedIndex + delta + gallery.length) % gallery.length;
  ...
  const add = () => {
    ...
    else items.push({ product_id: product.id, slug: product.slug, name: product.name, price: product.price, image_url: currentImage, quantity: 1, size });
  ...
  return (
    <main className={`product-page tone-${product.tone}`}>
      <header className="topbar product-top">
        <a className="wordmark" href="/">LE SHE<br /><span>SAREE</span></a>
        <nav><a href="/">Shop</a><a href="/account">Account</a><a href="/bag">Bag ({bag})</a></nav>
        <span className="top-meta">PRODUCT / {product.slug.toUpperCase()}</span>
      </header>
      ...
      <section className="product-detail">
        <div className="product-visual">
          <div className="product-gallery">
            {hasGallery && (
              <div className="product-thumbs" aria-label="Product image thumbnails">
                {gallery.map((item, index) => (
                  <button type="button" key={item.id} className={selectedImage === item.id ? "selected" : ""} onClick={() => setSelectedImage(item.id)} aria-label={`View image ${index + 1}`}>
                    <img src={item.image_url} alt={item.alt_text || product.name} />
                  </button>
                ))}
              </div>
            )}
            <div className="product-stage">
              <span className="product-gallery-count">{galleryLabel}</span>
              {currentImage ? (
                <img key={imageKey} src={currentImage} alt={gallery[selectedIndex]?.alt_text || product.name} />
              ) : (
                <div className="product-stage-empty"><span>{product.label}</span></div>
              )}
              {gallery.length > 1 && <>
                <button className="gallery-arrow prev" type="button" onClick={() => selectRelative(-1)} aria-label="Previous image">←</button>
                <button className="gallery-arrow next" type="button" onClick={() => selectRelative(1)} aria-label="Next image">→</button>
              </>}
            </div>
          </div>
        </div>
        ...
        <div className="product-info">
          ...
          {product.material && <p className="product-material"><b>MATERIAL</b> {product.material}</p>}
          <div className="option-label">SIZE <span>{size}</span></div>
          <div className="sizes">{product.sizes.map((item) => <button type="button" className={size === item ? "selected" : ""} onClick={() => setSize(item)} key={item}>{item}</button>)}</div>
          <button className={`add-button ${added ? "added" : ""}`} disabled={loading || product.stock <= 0} onClick={add}>{loading ? "LOADING…" : product.stock <= 0 ? "OUT OF STOCK" : added ? "ADDED TO BAG ✓" : "ADD TO BAG"}<span>↗</span></button>
          <div className="details"><p>{product.care_instructions || "Handcrafted textile · Gentle care recommended"}</p><p>{product.stock > 0 ? `${product.stock} available` : "Currently unavailable"}</p><p>Shipping calculated at checkout</p></div>
        </div>
      </section>
